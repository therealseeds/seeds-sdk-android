package com.playseeds.android.sdk.messaging;
...
public class CountlyMessaging extends WakefulBroadcastReceiver {
    ...
    /**
     * Action for Seeds Messaging BroadcastReceiver.
     * Once message is arrived, Seeds Messaging will send a broadcast notification with action "APP_PACKAGE_NAME.countly.messaging"
     ...
     * When message arrives, Seeds displays it either as a Notification, or as a AlertDialog. In any case,
    ...
    public static void init(Activity activity, Class<? extends Activity> activityClass, String sender, String[] buttonNames) {
        ...
        if (checkPlayServices(activity) ) {
            ...
            if (registrationId.isEmpty()) {
                ...
                Seeds.sharedInstance().onRegistrationId(registrationId);
            }
        } else {
            if (Seeds.sharedInstance().isLoggingEnabled()) {
    ...
    protected static boolean initCountly(Context context) {
        ...
        if (serverURL == null || appKey == null) {
            ...
            Seeds.sharedInstance().init(context, serverURL, appKey, deviceID, idMode);
            try {
    ...
    public static void recordMessageOpen(String messageId) {
        if (!Seeds.sharedInstance().isInitialized()) {
        ...
        Seeds.sharedInstance().recordEvent(EVENT_OPEN, segmentation, 1);
    }
    ...
    public static void recordMessageAction(String messageId) {
        ...
        Map<String, String> segmentation = new HashMap<String, String>();
        segmentation.put("i", messageId);
        Seeds.sharedInstance().recordEvent(EVENT_ACTION, segmentation, 1);
    }
}
